...
router.get('/manage', async function (req, res, next) {
	...
	let [reservations, fields] = await slot.eventSlotResv(213);
...
router.post('/create', async function (req, res, next) {
	...
	while (processedSlots < numSlots) {
		if (req.body['slotStart' + i]) {
			...
			let [end_date, end_time] = await helpers.parseDateTimeString(end);	//convert start date/time to MySQL-compatible format
